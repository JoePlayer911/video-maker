        ...
        for i in range(nb_frames):
            ...
            if keyframe:
                ...
                if last_keyframe["faces"] > 0:
                    ...
                    if speaker:
                        speaker_id = \
                            [x for x, key in enumerate(last_keyframe["faces_properties"]) if key["speaker"] == True][0]

            print("[INFO] saving: " + str(i) + " of " + str(nb_frames) + " - ", end="\r")
            frame = all_frames[i % len(all_frames)]
            if i not in no_face_frames and i < len(mel_chunks) + self.sound_start_frame:
                p, f, c, target = preds[current_frame], pred_frame[current_frame], pred_coords[current_frame], \
                                      pred_targets[current_frame]
                y1, y2, x1, x2 = c
                p = cv2.resize(p.astype(np.uint8), (x2 - x1, y2 - y1))
                #cv2.imwrite(f"{self.project_folder}/debug/w2l{i}.jpg", p)
                # From Here
                if self.video_quality in ["Medium", "High"]:
                    ...
                    #cv2.imwrite(f"{self.project_folder}/debug/w2l_r{i}.jpg", p)
                    #apply restored face to original image if rgb pixel in p is not black
